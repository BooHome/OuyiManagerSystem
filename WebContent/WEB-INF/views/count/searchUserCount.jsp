...
<form class="form-inline definewidth m20" action="searchuser" method="get">
    ...
    <div style="position: relative;top: 10px;left: 30px;">
	 ...
	 <div style="float:right">
		 <nav>
			<ul class="pager">
				<li><a href="searchuser?pageNum=1&startTime=${searchU['startTime']}
						&endTime=${searchU['endTime']}&keyName=${searchU['uTel']}&pNumber=${searchU['pNumber']}">首页</a></li>
				<c:if test="${pageInfo.hasPreviousPage}">
					<li><a href="searchuser?pageNum=${pageInfo.pageNum-1}&startTime=${searchU['startTime']}
						&endTime=${searchU['endTime']}&keyName=${searchU['uTel']}&pNumber=${searchU['pNumber']}" aria-label="Previous"> <span
						aria-hidden="true">上一页</span>
				</a></li>
				</c:if>
				<c:if test="${pageInfo.hasNextPage}">
					<li><a href="searchuser?pageNum=${pageInfo.pageNum+1}&startTime=${searchU['startTime']}
						&endTime=${searchU['endTime']}&keyName=${searchU['uTel']}&pNumber=${searchU['pNumber']}" aria-label="Next"> 
					<span aria-hidden="true">下一页</span>
				</a></li>
				</c:if>
				<li><a href="searchuser?pageNum=${pageInfo.pages}&startTime=${searchU['startTime']}
						&endTime=${searchU['endTime']}&keyName=${searchU['uTel']}&pNumber=${searchU['pNumber']}">尾页</a></li>
				<li>
					<form action="searchuser" method="get" style="float:right">
					  <input type="hidden" name="startTime" id="startTime" value="${searchU['startTime']}" >
					   <input  type="hidden" name="endTime" id="endTime"   value="${searchU['endTime']}" >
						<input type="hidden" name="keyName" id="keyName" value="${searchU['uTel']}">
						<input type="hidden" name="pNumber" id="pNumber" value="${searchU['pNumber']}">
